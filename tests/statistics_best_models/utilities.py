...
MODELS_FOLDER = os.path.join(os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "models")))
...
def extract_ravdess(gender=False):
    ...
    RAV_df = RAV_df.drop(labels=['emotion', 'voc_channel', 'modality', 'intensity', 'phrase'], axis=1)
    ...
    if gender: 
        ...
        RAV_test_f.drop(labels=['actors'], axis=1)
        RAV_test_f.reset_index(drop=True, inplace = True)
        RAV_test_m.drop(labels=['actors'], axis=1)
        RAV_test_m.reset_index(drop=True, inplace = True)

        

    RAV_train = RAV_train.drop(labels=['actors'], axis=1)
    RAV_val = RAV_val.drop(labels=['actors'], axis=1)
    RAV_test = RAV_test.drop(labels=['actors'], axis=1)
...
def extract_savee():
    ...
    SAVEE_train = SAVEE_train.drop(labels=['actors'], axis=1)
    SAVEE_val = SAVEE_val.drop(labels=['actors'], axis=1)
    SAVEE_test = SAVEE_test.drop(labels=['actors'], axis=1)
...
def extract_tess():
    ...
    TESS_train = TESS_train.drop(labels=['actors'], axis=1)
    TESS_test = TESS_test.drop(labels=['actors'], axis=1)
...
def extract_crema(gender=False):
    ...
    CREMA_train = CREMA_train.drop(labels=['actors'], axis=1)
    CREMA_val = CREMA_val.drop(labels=['actors'], axis=1)
    if gender: 
        CREMA_test_f = CREMA_test_f.drop(labels=['actors'], axis=1)
        CREMA_test_m = CREMA_test_m.drop(labels=['actors'], axis=1)
    else:
        CREMA_test = CREMA_test.drop(labels=['actors'], axis=1)
...
def make_test(df_test, model_list, test_type): 
    for model in model_list: 
        ...
        print("Resulting Dataframe", df_res)
        print("Model Name")
        print("###### ", model, " ######")
        report = classification_report(df_res.true_label, df_res.pred_label, output_dict=True)
        df = pd.DataFrame(report).transpose()
        pathname = os.path.join(os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "results", test_type, model + ".json")))
        results_folder = os.path.join(os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "results", test_type)))
        # create a folder for the results if it does not exist
        if not os.path.exists(results_folder):
            os.makedirs(results_folder)
        # save df as json with tab 4 
        df.to_json(pathname, indent=4)
